...
beforeAll(async () => {
  ...
  const agent = await auth.createPrincipal("agent", "rogue");
  agentId = agent.id;
...
describe("N6 — reversibility across compaction", () => {
  it("flags notes as `partial` when the actor's edits were compacted into a snapshot", async () => {
    ...
    const adminTok = (await auth.bootstrapAdmin())!.token;
    const bot = await auth.createPrincipal("agent", "rogue3");
    const botTok = await auth.issueToken(bot.id);
