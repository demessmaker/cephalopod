...
| [`mcp/`](mcp/) | M4–M4.1 ✅ | MCP server — 10 agent tools (incl. `get_context`, a token-budgeted retrieval bundle), note **resources**, **live subscriptions** (`resources/updated`), and guided **prompts** (`capture-decision`/`onboard`); the agent-facing surface. |
...
| [`web/`](web/) | M8 ✅ | Web graph explorer (north-star UI) — build-less: search → force-directed subgraph → click-to-expand → live refresh, a **review queue** for gated/agent notes (promote/reject), and per-note **history/blame**. |
